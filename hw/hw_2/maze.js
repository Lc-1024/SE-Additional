...
var padding = 16, s, density=0.5, count=3;
var queue = [0], tree = [];
...
function getMyNextStepForMaze1(index, sx, sy, a) {
    var n = [];
    ...
    for (let i = 0; i < quadSteps.length; i ++) {
        ...
        if(sx + step.dx > -1 && sx + step.dx < cols && sy + step.dy > -1 && sy + step.dy < rows &&
            mazes[index][sx + step.dx][sy + step.dy] % 8 == a){
            n.push({x: sx + quadSteps[i].dx, y: sy + quadSteps[i].dy});
        }
    }
...
function solveMaze1Mine(index) {
    var tmp = queue.length;

    for (var i = 0; i < tmp; ++i) {
        if( tree[queue[i]].x == end[index].x && tree[queue[i]].y == end[index].y ) {
            var t = queue[i];
            while ( t != -1 ) {
                mazes[index][tree[t].x][tree[t].y] = 3;
                t = tree[t].p;
            }
            for( var i = 0; i < cols; i++ ) {
                for( var j = 0; j < rows; j++ ) {
    ...
    for (var i = 0; i < tmp; ++i) {
        /*  we want to find the neighbours of tree[queue[i]], 
            if they are empty, we can go through this path, 
            and they are tree[queue[i]]'s children, c += 1.
           */
        var neighbours = getMyNextStepForMaze1( index, tree[queue[i]].x, tree[queue[i]].y, 0 );
        if( neighbours.length > 0 ) {
            tree[queue[i]].c += neighbours.length;
            for ( var j = 0; j < neighbours.length; ++j ) {
                queue.push(tree.length);
                tree.push({ x: neighbours[j].x, y: neighbours[j].y, c: 0, p: queue[i] });
                mazes[index][tree[queue[i]].x][tree[queue[i]].y] = 2;
            }
        } else {
            var t = queue[i];
            while (t > 0 && tree[t].c == 0) {
                mazes[index][tree[t].x][tree[t].y] = 4;
                t = tree[t].p;
                --tree[t].c;
            }
        }
...
function createMaze2(ctx) {
    ...
    mazes[2][start[0].x][start[0].y] = r < density ? 0 : 1;
